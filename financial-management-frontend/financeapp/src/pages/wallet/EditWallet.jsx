...
export default function EditWallet() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8080/api/wallets/${id}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
        .then(response => setWallet(response.data))
        .catch(error => setError("Không thể tải thông tin ví"));
    }, [id]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem('userToken');
            await axios.put(`http://localhost:8080/api/wallets/update/${id}`, wallet, {
                headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }
            });
            alert('Cập nhật ví thành công');
            navigate('/wallets');
        } catch (error) {
            setError('Không thể cập nhật ví, kiểm tra lại dữ liệu!');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Box sx={{ maxWidth: 400, mx: 'auto', mt: 5, p: 3, border: '1px solid #ddd', borderRadius: 2, boxShadow: 3 }}>
            ...
            <TextField fullWidth label="Số dư" name="balance" type="number" value={wallet.balance} onChange={handleChange} sx={{ mb: 2 }} />
            ...
            <TextField fullWidth label="Icon URL" name="iconUrl" value={wallet.iconUrl} onChange={handleChange} sx={{ mb: 2 }} />

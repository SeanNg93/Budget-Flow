...
-- adds the original_wallet_name column, and sets the foreign key constraint
...
-- Step 2: Add the original_wallet_name column if it doesn't exist
-- Use a suitable length, e.g., VARCHAR(100)
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS original_wallet_name VARCHAR(100) NULL;

-- Step 3: Find the existing foreign key constraint name for account_id.
...
-- Step 4: Drop the existing foreign key constraint (Replace 'YOUR_CONSTRAINT_NAME')
-- Example: ALTER TABLE transactions DROP FOREIGN KEY transactions_ibfk_2;
-- Make sure to uncomment the line below and replace YOUR_CONSTRAINT_NAME after running Step 3
...
-- Ensure this constraint name doesn't already exist if you ran parts of this before
ALTER TABLE transactions 
ADD CONSTRAINT fk_transactions_wallets_set_null 
FOREIGN KEY (account_id) REFERENCES wallets(id) ON DELETE SET NULL;

SELECT 'Schema update to allow NULL account_id and add original_wallet_name completed.' AS message;
